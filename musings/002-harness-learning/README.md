...
我们让 Agent 去跑一次 benchmark。这里先拿比较规范的终端任务基准 [Terminal-Bench 2.0](https://www.tbench.ai/leaderboard/terminal-bench/2.0) 来想。Agent 接到一个真实终端环境里的任务，打开 shell，调用工具，中间撞上几个报错，好不容易把结果交上去。Verifier 随后检查环境是否满足验收条件。几十道题跑完，排行榜上多了一个分数，实验目录里多了一堆日志。
...
修改方向还得从执行过程里找。Terminal-Bench 的 Verifier 不一定只丢回来一个光秃秃的 FAIL。具体任务可以提供单项测试结果、错误输出或部分得分，这些信息能指出哪条验收条件没有满足。

验收结果通常不会继续替 Agent 分析做题过程。同一项测试失败，可能是工具选错了，可能是关键输出没有检查，也可能是 Agent 离成功只差一步时提前宣布胜利。我们知道“哪里没验过”，还得回到 trajectory 里寻找“为什么会走到这里”。
...
有人会把这种诊断叫作 `semantic gradient`。这个名字借用了梯度“指示更新方向”的感觉。真正的梯度是一组可以参与计算的数值，能告诉优化器参数怎样移动；这里得到的是一句自然语言诊断，例如“关键修改以后应当验证目标状态”。它只能给 Evolver 一个修改方向，无法保证沿着这个方向改就一定涨分。方向找到了，文件还没有动，真正落笔修改 Harness 的工作要交给 Evolver。
