...
我把第一种叫作开卷考，因为模型的脑子里可以没有新增东西。每次任务开始前，Agent 给它准备一份与这场考试对应的参考资料，也就是一张 cheat sheet。模型现场翻、现场查，任务结束后，参数还是原来的参数。

第二种叫闭卷考。当前任务仍然会作为 Prompt 交给模型，只是过去那份经验不必再充当外部参考资料。训练把新的行为倾向或能力写进参数，下一次即使不把旧经验重新塞进 Prompt，模型也可能做出不同的选择。
...
第一种是给每场任务准备对应的 cheat sheet。模型自己不必记住用户偏好和项目规则，只要 Agent 在考试前找对材料，再把它们放进上下文。
...
第二种要把练习留下的东西写进模型参数。考试时不再现场翻阅那份经验，模型依靠训练后的参数作答。这里说的是参数学习，LoRA 是更适合 Agent 版本管理的一种实现。
